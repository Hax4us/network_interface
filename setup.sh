...
xterm -T "☣ INSTALL SEARCHSPLOIT ☣" -geometry 100x30 -e "apt-get install exploitdb --force-yes -y"
which searchsploit > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
...
#apt-get clean
#xterm -T "☣ UPDATE YOUR REPO ☣" -geometry 100x30 -e "apt-get update "
#clear
...
#xterm -T "☣ INSTALL BACKDOOR-FACTORY ☣" -geometry 100x30 -e "apt-get install backdoor-factory --force-yes -y"
#which backdoor-factory > /dev/null 2>&1
#if [ "$?" -eq "0" ]; then
...
#xterm -T "☣ INSTALL METASPLOIT-FRAMEWORK ☣" -geometry 100x30 -e "apt-get install metasploit-framework --force-yes -y"
which msfconsole > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
...
echo "☣ INSTALL Ruby ☣"apt-get install ruby -y && gem install nokogiri -- --use-system-libraries
which ruby >> "$log" 2>&1
if [ "$?" -eq "0" ]; then
...
apt-get install imagemagick imagemagick-dev python2 python2-dev libllvm libllvm-dev libc++ -y && pip2 install names
sleep 1
...
ln -s "$path/tools/android-sdk/dx" "$PREFIX/bin/dx" > /dev/null 2>&1
which dx > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
which dx >> "$log" 2>&1
echo "dx" | tee -a "$config" >> /dev/null 2>&1
...
echo "☣ Removing Your Current Aapt ☣"
apt-get remove aapt -y
#unlink "/usr/local/sbin/aapt" > /dev/null 2>&1
ln -s "$path/tools/android-sdk/aapt" "$PREFIX/bin/aapt" > /dev/null 2>&1
which aapt > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
which aapt >> "$log" 2>&1
echo "aapt" | tee -a "$config" >> /dev/null 2>&1
...
unlink "$PREFIX/bin/aapt" > /dev/null 2>&1
ln -s "$path/tools/android-sdk/aapt" "$PREFIX/bin/aapt" > /dev/null 2>&1
which aapt > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
which aapt >> "$log" 2>&1
...
ln -s "$HOME/apktool/apktool" "$PREFIX/bin/apktool" > /dev/null 2>&1
which apktool > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
echo -e $green "[ ✔ ] Apktool v.2.2.2"
which apktool >> "$log" 2>&1
...
cp $path/tools/dex2jar/* $PREFIX/bin/ > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-baksmali > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-dex-recompute-checksum > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-dex2jar > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-dex2smali > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-jar2dex > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-jar2jasmin > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-jasmin2jar > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-smali > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-std-apk > /dev/null 2>&1
...
mkdir $PREFIX/share/dex2jar > /dev/null 2>&1
cp -r $path/tools/dex2jar/lib $PREFIX/share/dex2jar/lib > /dev/null 2>&1
which d2j-dex2jar > /dev/null 2>&1
...
chmod +x $PREFIX/bin/d2j-smali > /dev/null 2>&1
chmod +x $PREFIX/bin/d2j-std-apk > /dev/null 2>&1
#rm -rf $PREFIX/share/dex2jar > /dev/null 2>&1
mkdir $PREFIX/share/dex2jar > /dev/null 2>&1
cp -r $path/tools/dex2jar/lib $PREFIX/share/dex2jar/lib > /dev/null 2>&1
which d2j-dex2jar > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
echo -e $green "[ ✔ ] Dex2Jar 2.0"
which d2j-dex2jar >> "$log" 2>&1
echo "d2j-dex2jar" | tee -a "$" >> /dev/null 2>&1
echo "Dex2Jar -> OK" >> "$inst"
else
echo -e $red "[ x ] Dex2Jar 2.0"
echo "0" > "$stp"
...
dir=`pwd` 
scrp="cd $dir && ./fatrat"
rm -f $PREFIX/bin/fatrat
touch $PREFIX/bin/fatrat
echo "#!/bin/bash" > $PREFIX/bin/fatrat
echo $scrp >> $PREFIX/bin/fatrat
cp $path/config/TheFatRat.desktop $PREFIX/share/applications/TheFatRat.desktop
cp $path/icons/fatrat.ico $PREFIX/share/icons/fatrat.ico
chmod +x $PREFIX/bin/fatrat
chmod +x fatrat
chmod +x update
