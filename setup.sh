...
echo "☣ INSTALL Ruby ☣"
apt-get install ruby -y && gem install nokogiri -- --use-system-libraries
which ruby >> "$log" 2>&1
if [ "$?" -eq "0" ]; then
...
apt-get install openssl openssl-tool -y
which openssl >> "$log" 2>&1
if [ "$?" -eq "0" ]; then
...
apt-get install imagemagick imagemagick-dev libllvm libllvm-dev ndk-multilib libc++ python2 python2-dev -y && pip2 install names
sleep 1
...
rm $PREFIX/bin/aapt >/dev/null 2>&1
...
ln -s "$path/tools/apktool2.2.2/apktool" "$PREFIX/bin/apktool" > /dev/null 2>&1
which apktool > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
echo -e $green "[ ✔ ] Apktool v.2.2.2"
which apktool >> "$log" 2>&1
...
unlink "$PREFIX/bin/apktool" > /dev/null 2>&1
ln -s "$path/tools/apktool2.2.2/apktool" "$PREFIX/bin/apktool" > /dev/null 2>&1
which apktool > /dev/null 2>&1
if [ "$?" -eq "0" ]; then
which apktool >> "$log" 2>&1
...
echo "#!/data/data/com.termux/files/usr/bin/bash" > $PREFIX/bin/fatrat
...
clear
echo ""
echo -e $green "Instalation completed , To execute fatrat write anywhere in your tefatra fatrat"
fi
;;
